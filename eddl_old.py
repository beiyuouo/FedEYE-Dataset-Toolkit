...
def main():
    ...
    for i in range(num_clients):
        ...
        client_data[i] = client_data[i].rename(
            columns={"category": "label", "name": "original_name"}
        )

        client_data[i].drop(columns=["type", "grade"], inplace=True)
        client_data[i].label = client_data[i].label.astype(int)
        client_data[i].original_name = (
            client_data[i].original_name.astype(str).str.replace(".jpg", "")
        )

        # reindex the name column
        client_data[i].reset_index(drop=True, inplace=True)
        # create name column
        client_data[i]["name"] = client_data[i].index.astype(str)
        # client_data[i].columns = ["name", "label", "original_name"]
        client_data_csv = client_data[i].copy()
        client_data_csv.drop(columns=["original_name"], inplace=True)
        client_data_csv = client_data_csv[["name", "label"]]

        client_data_csv.to_csv(
            os.path.join(client_export_root, "train.csv"),
            index=False,
        )
            ...
            yaml.dump(
                {
                    "attributes": {
                        "evalType": "multi",
                        "name": f"Eye Disease Deep Learning Dataset {i} of {num_clients}".replace(
                            " ", "_"
                        ),
                        "note": f"This dataset is generated by EDDLD dataset with {iid if iid else f'non-iid alpha={alpha}'} partitioning.".replace(
                            " ", "_"
                        ),
                    },
                    "inputs": {
                    ...
                    "labelMappping": [
                        {
                            "label": "0",
                            "mapped": "point-like corneal ulcers".replace(" ", "_"),
                        },
                        {
                            "label": "1",
                            "mapped": "point-flaky mixed corneal ulcers".replace(
                                " ", "_"
                            ),
                        },
                        {
                            "label": "2",
                            "mapped": "flaky corneal ulcers".replace(" ", "_"),
                        },
                    ],
                    "targets": [
        ...
        for idx, image_nm in enumerate(client_data[i].original_name):
            image_nm = image_nm + ".jpg"
            shutil.copy(
                os.path.join(image_root, image_nm),
                os.path.join(
                    client_export_root, "images", client_data[i].name[idx] + ".jpg"
                ),
            )

        # zip file
        shutil.make_archive(client_export_root, "zip", client_export_root)
